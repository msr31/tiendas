<template>
  <v-dialog persistent v-model="mostrar">
    <v-card>
      <v-card-title class="headline">Abonos del apartado #{{apartado.IdApartado}}</v-card-title>
      <v-card-text>
        <v-layout row wrap>
          <v-flex xs12>
            ...
            <v-text-field ref="inputPago" v-show="restante > 0 && !estaVencido" @keyup.enter.prevent="abonar()"
                          v-model.number="pago" prepend-icon="monetization_on"
                          label="Pago del cliente" type="text" hint="Cantidad que da el cliente"
                          required></v-text-field>
            <v-text-field ref="inputAbono" v-show="restante > 0 && !estaVencido" @keyup.enter.prevent="abonar()"
                          v-model.number="cantidadAbonada" prepend-icon="monetization_on"
                          label="Cantidad a abonar" type="text" hint="Del pago del cliente, ¿Cuánto se abona?"
                          required></v-text-field>
            <v-flex xs12>
              <p class="title">Pago: {{ pago | currency }}</p>
              <p class="title">Abono: {{ cantidadAbonada | currency }}</p>
              <p class="title" v-show="cambio >= 0">Cambio: {{ cambio | currency }}</p>
              <p class="title" v-show="cambio >= 0">Restante: {{ restante - cantidadAbonada | currency }}</p>
            </v-flex>
          </v-flex>
          <v-flex xs12>
            <v-btn title="Haga click aquí para realizar el abono" :loading="cargandoAbonando"
                   v-show="cantidadAbonada > 0" @click="abonar()" small color="success">
              Terminar abono
            </v-btn>
          </v-flex>
          <v-flex xs12 sm6>
          ...
          <v-flex xs12>
            <v-data-table :headers="encabezados" :items="abonos" hide-actions item-key="props.item.index">
              <template slot="items" slot-scope="props">
                <tr>
                  ...
                  <td class="justify-center layout px-0">
                    <v-btn title="Detalles" icon class="mx-0" @click="imprimir(props.item)">
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="green darken-1" flat="flat" @click.native="ocultarDialogo()">Cerrar</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
