<template>
  <v-layout row wrap>
    <seleccionador-fechas @cambio="comprobarFechasYRefrescarSiEsNecesario"></seleccionador-fechas>
    <v-flex xs12>
    ...
    <v-flex xs12>
      <v-data-table :headers="encabezadosVentas" :items="ventas" hide-actions item-key="IdVenta">
        <template slot="items" slot-scope="props">
          <tr class="cursor-manita" :class="{expandido: props.expanded}" @click="props.expanded = !props.expanded">
            ...
            <td class="justify-center layout px-0">
              <v-btn title="Reimprimir ticket" icon class="mx-0" @click="imprimirTicket(props.item.IdVenta)">
                <v-icon :color="props.expanded ? 'white' : 'blue'">print</v-icon>
              </v-btn>
            </td>
          </tr>
        </template>
        <template slot="expand" slot-scope="props">
          <v-card flat class="productos-vendidos">
            <v-data-table :headers="encabezadosProductos" :items="props.item.productos" hide-actions item-key="Numero">
              <template slot="items" slot-scope="props">
                <tr>
                  ...
                  <td>
                    <strong>{{ props.item.PrecioVenta - props.item.PrecioCompra | currency }}</strong>
                  </td>
                  <td>{{ props.item.Cantidad }}</td>
                  <td>{{ props.item.PrecioVenta * props.item.Cantidad | currency }}</td>
                </tr>
              </template>
            </v-data-table>
          </v-card>
  ...
  export default {
    ...
    methods: {
      imprimirTicket(idVenta) {
        this.$router.push({name: "TicketDeVentaContado", params: {idVenta}});
      },
