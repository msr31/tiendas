<template>
  <v-dialog v-model="mostrar" persistent max-width="700">
    <v-card>
      <v-card-title class="headline">Cambiando fecha de vencimiento</v-card-title>
      <v-card-text>
              ...
              <v-flex xs12>
                ...
                >
                  <v-text-field
                    ...
                    readonly
                  ></v-text-field>
                  <v-date-picker color="green lighten-1" locale="es-419"
                                 v-model="nuevaFechaVencimiento" :min="hoy">
                    <v-spacer></v-spacer>
                    <v-btn flat color="primary" @click="mostrarDialogoFecha = false">Cerrar</v-btn>
                    <v-btn flat color="primary" @click="$refs.menu.save(nuevaFechaVencimiento)">OK</v-btn>
                  </v-date-picker>
                </v-menu>
              </v-flex>
            </v-layout>
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn :loading="cargando" color="green darken-1" flat="flat" @click.native="guardar()">Guardar
        </v-btn>
