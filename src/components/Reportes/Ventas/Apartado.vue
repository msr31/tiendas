<template>
  <v-layout row wrap>
    ...
    <seleccionador-fechas v-show="tipoReporte === 'todos'"
    ...
    <v-flex xs12>
      <v-layout row wrap>
        <v-flex xs12 sm6>
          <v-text-field v-model="numeroApartadoParaAbonar"
                        ref="nombreCliente"
    ...
    <abonos @abonado="onApartadoAbonado" @liquidar="snackbars.entregarProductos = true"
            @cantidadSuperior="snackbars.cantidadSuperior = true" @cantidadNegativa="snackbars.cantidadNegativa = true"
            :apartado="apartadoSeleccionado" :mostrar="dialogos.abonos" @cerrar="dialogos.abonos = false">
    ...
    <v-flex xs12>
      <v-radio-group label="Mostrar..." v-model="tipoReporte" row>
        ...
        <v-radio label="Próximos a vencer (1 semana)" value="proximos"></v-radio>
      </v-radio-group>
    </v-flex>
    <v-flex xs12>
      <v-data-table :headers="encabezadosApartados" :items="apartados" hide-actions item-key="IdApartado">
        <template slot="items" slot-scope="props">
          <tr>
            ...
            <td>{{ props.item.Monto - props.item.Anticipo - props.item.Abonado | currency }}</td>
            ...
            <td class="justify-center layout px-0">
              <v-btn title="Cambiar fecha de vencimiento" icon class="mx-0"
                     @click="cambiarFechaVencimiento(props.item)">
                <v-icon color="error">update</v-icon>
              </v-btn>
              <v-btn title="Detalles" icon class="mx-0" @click="mostrarDetalles(props.item)">
                <v-icon color="blue">info</v-icon>
              </v-btn>
              <v-btn title="Abonar" icon class="mx-0" @click="mostrarAbonos(props.item)">
                <v-icon color="green">monetization_on</v-icon>
              </v-btn>
              <v-btn title="Imprimir ticket" icon class="mx-0" @click="imprimir(props.item)">
    ...
    <v-snackbar :timeout="2000" :top="true" v-model="snackbars.cantidadNegativa">
      La cantidad abonada debe ser mayor que cero
      <v-btn flat color="pink" @click.native="snackbars.cantidadNegativa = false">OK</v-btn>
    </v-snackbar>
    <v-snackbar :timeout="2000" :top="true" v-model="snackbars.cantidadSuperior">
      Introduce una cantidad menor o igual al restante
      <v-btn flat color="pink" @click.native="snackbars.cantidadSuperior = false">OK</v-btn>
    </v-snackbar>
    <v-snackbar :timeout="0" :top="true" v-model="snackbars.entregarProductos">
      Apartado liquidado. Ahora puede entregar los productos
      <v-btn flat color="pink" @click.native="snackbars.entregarProductos = false">OK</v-btn>
    </v-snackbar>
    <v-snackbar :timeout="3000" :bottom="true" v-model="snackbars.fechaVencimientoCambiada">
      Fecha de vencimiento cambiada correctamente
      <v-btn flat color="pink" @click.native="snackbars.fechaVencimientoCambiada = false">OK</v-btn>
    </v-snackbar>
    <v-snackbar :timeout="3000" :bottom="true" v-model="snackbars.numeroApartadoInexistenteOErroneo">
      No existe un apartado con ese número
      <v-btn flat color="pink" @click.native="snackbars.numeroApartadoInexistenteOErroneo = false">OK</v-btn>
    </v-snackbar>
  </v-layout>
</template>
<script>
