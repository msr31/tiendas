<template>
  <v-layout row wrap>
      ...
      <v-slide-y-transition>
        ...
        >
          <template slot="selection" slot-scope="data">
            <v-subheader>{{data.item.Nombre}} - #{{data.item.Numero}}</v-subheader>
          </template>
    ...
    <v-flex xs12>
      <v-btn :loading="cargando" @click="imprimir" small color="success">Imprimir</v-btn>
    </v-flex>
  </v-layout>
</template>
<script>
        ...
        if (this.tipoReporte === "usuario" && this.usuarioSeleccionado.Numero) {
          this.$router.push({
            ...
            query: {
              ...
              usuario: this.usuarioSeleccionado.Numero
            }
          });
        } else {
          this.$router.push({
