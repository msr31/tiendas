  ...
  <v-container fluid fluid-xs>
    ...
    <v-slide-y-transition>
      <v-layout row wrap v-show="mostrar">
        <v-flex xs12 sm3>
          <v-menu ref="menuFechaInicio" :close-on-content-click="false" v-model="menuFechaInicio" :nudge-right="40"
                  :return-value.sync="fechaInicio"
                  lazy transition="scale-transition" offset-y full-width max-width="290px" min-width="290px">
            <v-text-field slot="activator" v-model="fechaInicio" label="Ver desde" prepend-icon="event"
                          readonly></v-text-field>
            <v-date-picker locale="es-419" v-model="fechaInicio" scrollable>
              <v-spacer></v-spacer>
              <v-btn flat color="primary" @click="menuFechaInicio = false">Cancelar</v-btn>
              <v-btn flat color="primary" @click="guardarFechaInicio()">OK</v-btn>
            </v-date-picker>
          </v-menu>
        </v-flex>
        <v-flex xs12 sm3>
          <v-menu ref="menuFechaFin" :close-on-content-click="false" v-model="menuFechaFin" :nudge-right="40"
                  :return-value.sync="fechaFin"
                  lazy transition="scale-transition" offset-y full-width max-width="290px" min-width="290px">
            <v-text-field slot="activator" v-model="fechaFin" label="Hasta" prepend-icon="event"
                          readonly></v-text-field>
            <v-date-picker locale="es-419" v-model="fechaFin" scrollable>
              <v-spacer></v-spacer>
              <v-btn flat color="primary" @click="menuFechaFin = false">Cancelar</v-btn>
              <v-btn flat color="primary" @click="guardarFechaFin()">OK</v-btn>
            </v-date-picker>
          </v-menu>
        </v-flex>
        <v-flex xs12 sm6>
          <v-btn @click="hoy()" :flat="seleccionado === 'hoy'" small
                 :color="seleccionado === 'hoy' ? 'success' : 'primary'">Hoy
            <v-icon v-show="seleccionado === 'hoy'">check</v-icon>
          </v-btn>
          <v-btn @click="semana()" :flat="seleccionado === 'semana'" small
                 :color="seleccionado === 'semana' ? 'success' : 'primary'">Esta semana
            <v-icon v-show="seleccionado === 'semana'">check</v-icon>
          </v-btn>
          <v-btn @click="mes()" :flat="seleccionado === 'mes'" small
                 :color="seleccionado === 'mes' ? 'success' : 'primary'">Este mes
