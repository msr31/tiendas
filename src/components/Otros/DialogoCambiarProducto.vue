    ...
    <v-card>
      ...
      <v-card-text>
            ...
            <v-flex xs12 sm6>
              <v-alert :value="nuevoProducto.Numero" type="success">
                ...
                <p>
                  ...
                  <br>
                  <strong>Precio:</strong> {{nuevoProducto.PrecioVenta | currency}}
                </p>
              </v-alert>
            </v-flex>
          </v-layout>
        ...
        <v-alert :value="errores.precioDeVentaMenor" type="error">
          No puede seleccionar un producto con un precio menor al que desea cambiar
        </v-alert>
        <v-alert :value="errores.noEncontradoPorCodigoONumero" type="error">
          No existe un producto con el código buscado. Intente de nuevo o busque por descripción
        </v-alert>
        <span class="body-2">{{etiquetaLectura}}</span>
        <v-container>
          <v-layout>
            <v-flex xs12>
              <v-text-field solo v-model="codigoONumero" @keyup.enter="buscarProductoPorCodigoONumero(codigoONumero)"
                            :readonly="cargando"
                            label="Escanear, o escribir y presionar Enter"
                            :loading="cargando"
                            ref="codigoProducto" type="text"></v-text-field>
            </v-flex>
          </v-layout>
        </v-container>
        <br>
        ...
        <autocompletado-productos
          @producto-seleccionado="onProductoSeleccionadoDesdeAutocompletado"></autocompletado-productos>
      </v-card-text>
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn :disabled="!nuevoProducto.Numero" :loading="cargando" color="green darken-1" flat="flat"
               @click.native="confirmarCambio()">Confirmar cambio
        </v-btn>
