            ...
            <v-layout row wrap>
              <v-flex xs12>
                <v-text-field @keyup.enter="guardar()" ref="monto" prepend-icon="monetization_on" label="Monto"
                              type="number" v-model.number="egreso.monto"
                              :rules="reglas.monto" hint="La cantidad que sale" required></v-text-field>
              </v-flex>
              <v-flex xs12>
                <v-text-field @keyup.enter="guardar()" prepend-icon="note_add" label="Descripción" type="text"
                              v-model="egreso.descripcion"
                              :rules="reglas.descripcion" hint="La descripción del egreso o salida"
