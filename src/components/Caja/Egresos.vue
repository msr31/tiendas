<template>
  <v-layout row wrap>
    ...
    <v-flex xs12>
      <v-data-table :headers="encabezados" :items="egresos" hide-actions item-key="props.item.index">
  ...
  export default {
    methods: {
      obtener(inicio, fin) {
         ...
         */
        this.total = 0;
        HTTP_AUTH.get(`egresos/${inicio}/${fin}`).then(egresos => {
          this.total = egresos.reduce(
