      ...
      <v-card-text>
        <v-form ref="formulario">
          <v-container fluid grid-list-md>
            <v-layout row wrap>
              <v-flex xs12 sm4>
                <v-text-field @keydown.native.enter="guardar()" ref="pago" prepend-icon="attach_money"
                              label="Pago del cliente" type="number" v-model.number="pago"
                              :rules="reglas.pago" hint="¿Con cuánto paga el cliente (para calcular el cambio)?"
                              required></v-text-field>
              </v-flex>
              <v-flex xs12 sm4>
                <v-text-field @keydown.native.enter="guardar()" ref="anticipo" prepend-icon="attach_money"
                              label="Anticipo" type="number" v-model.number="anticipo"
                              :rules="reglas.anticipo" hint="¿Cuál es el anticipo del cliente?" required></v-text-field>
              </v-flex>
              <v-flex xs12 sm4>
                <v-menu
                  ref="menu"
                ...
                >
                  <v-text-field
                    ...
                    readonly
                  ></v-text-field>
                  <v-date-picker color="green lighten-1" :events="fechasRecomendadas" locale="es-419"
                                 v-model="fechaVencimiento" :min="hoy">
                    <v-spacer></v-spacer>
                    <v-btn flat color="primary" @click="mostrarDialogoFecha = false">Cerrar</v-btn>
                    <v-btn flat color="primary" @click="$refs.menu.save(fechaVencimiento)">OK</v-btn>
                  </v-date-picker>
                </v-menu>
              </v-flex>
            </v-layout>
        ...
        <detalles-cliente-seleccionado :clienteSeleccionado="clienteSeleccionado"></detalles-cliente-seleccionado>
        <autocompletado-clientes ref="autocompletado" @cliente-cancelado="onClienteCancelado"
                                 @cliente-seleccionado="onClienteSeleccionado"
                                 @agregar-cliente="agregarNuevoCliente"></autocompletado-clientes>
        <br>
        <v-flex xs12>
          <p class="title text-xs-right" color="blue">Total: {{datosVenta.total | currency}}</p>
          <p v-show="pago > 0 && cambio >= 0" class="title text-xs-right" color="blue">Pago: {{pago | currency}}</p>
          <p v-show="pago > 0 && cambio >= 0" class="title text-xs-right" color="blue">Anticipo: {{anticipo |
            currency}}</p>
          <p v-show="cambio >= 0" class="title text-xs-right" color="blue">Cambio: {{cambio | currency}}</p>
          <p v-show="cambio >= 0" class="title text-xs-right" color="blue">Restante: {{datosVenta.total - anticipo |
            currency}}</p>
        </v-flex>
      </v-card-text>
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn :loading="cargando" color="green darken-1" flat="flat" @click.native="guardar()">Terminar apartado
        </v-btn>
    ...
    methods: {
      ...
      guardar() {
        if (this.$refs.formulario.validate()) {
          ...
          this.cargando = true;
          HTTP_AUTH.post("apartado", apartado).then(resultados => {
            this.cargando = false;
            if (resultados) {
