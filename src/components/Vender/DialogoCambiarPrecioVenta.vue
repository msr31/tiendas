<template>
  <v-dialog v-model="mostrar" persistent max-width="500">
    <v-card>
      ...
      <v-card-text>
        <p><strong>Precio original: </strong>{{producto.PrecioVentaOriginal | currency}}</p>
        <v-form ref="formulario">
          <v-container fluid grid-list-md>
            <v-layout row wrap>
              <v-flex xs12>
                <v-text-field @keydown.prevent.native.enter="guardarNuevoPrecio" :rules="reglas.nuevoPrecio"
                              ref="nuevoPrecio"
                              prepend-icon="attach_money"
                              label="Nuevo precio de venta" type="number"
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="gray" flat="flat" @click.native="cerrarDialogo()">Cancelar</v-btn>
        <v-btn color="green darken-1" flat="flat" @click="guardarNuevoPrecio()">Ok</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
