<template>
  <v-dialog v-model="mostrar" persistent max-width="700">
    <v-card>
      ...
      <v-card-text>
        <v-form ref="formulario">
          <v-container fluid grid-list-md>
            <v-layout row wrap>
              <v-flex xs12>
                <v-text-field @keyup.enter="guardar()" ref="pagoCliente" prepend-icon="attach_money"
                              label="Cantidad recibida" type="number" v-model.number="pagoDelCliente"
                              :rules="reglas.pago" hint="¿Con cuánto paga el cliente?" required></v-text-field>
              </v-flex>
            </v-layout>
          </v-container>
          <v-flex xs12>
            ...
            <v-radio-group v-model="tipoCliente" row>
              ...
              <v-radio label="Buscar o crear nuevo" value="existenteONuevo"></v-radio>
            </v-radio-group>
          </v-flex>
        ...
        <v-flex xs12>
          ...
          <p v-show="pagoDelCliente > 0" class="title text-xs-right" color="blue">Pago: {{pagoDelCliente |
            currency}}</p>
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn :loading="cargando" color="green darken-1" flat="flat" @click.native="guardar()">Terminar venta</v-btn>
    ...
    methods: {
      ...
      guardar() {
        if (this.$refs.formulario.validate()) {
          ...
          this.cargando = true;
          HTTP_AUTH.post("venta/contado", venta).then(resultados => {
            this.cargando = false;
