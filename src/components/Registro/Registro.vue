        ...
        <v-card-text>
          <v-form ref="formulario">
            <v-container fluid grid-list-md>
              <v-layout row wrap>
                <v-flex xs12>
                  <v-text-field @keyup.enter="intentarRegistrar()" :rules="reglas.correo"
                                prepend-icon="email" label="Tu correo electrónico"
                                type="text"
                                hint="Ahí te enviaremos un mensaje de confirmación"
                                v-model="negocio.correo"
                                required></v-text-field>
                </v-flex>
                <v-flex xs12>
                  <v-text-field @keyup.enter="intentarRegistrar()" :rules="reglas.negocio"
                                prepend-icon="store_mall_directory" label="Nombre de tu negocio"
                                type="text"
                                v-model="negocio.nombre"
                                required></v-text-field>
                </v-flex>
                <v-flex xs12>
                  <v-text-field @keyup.enter="intentarRegistrar()" :rules="reglas.pass"
                                prepend-icon="lock" label="Escribe una contraseña"
                                type="password"
                ...
                <v-flex xs12>
                  <v-text-field @keyup.enter="intentarRegistrar()" :rules="reglas.pass"
                                prepend-icon="lock" label="Confirma tu contraseña"
                                type="password"
                ...
                <v-flex xs12>
                  <v-checkbox
                    v-model="negocio.aceptaCondiciones"
                    :rules="reglas.condiciones"
              ...
              <v-btn :loading="cargando" color="success" @click="intentarRegistrar()">Registrarme</v-btn>
            </v-container>
          </v-form>
          <v-btn @click="irAlLogin()" small flat :loading="cargando" color="info">Ya tengo una cuenta</v-btn>
        </v-card-text>
      </v-card>
    </v-flex>
    <v-snackbar :timeout="3000" :bottom="true" v-model="snackbarPassNoCoincide">
      Las contraseñas no coinciden
      <v-btn flat color="pink" @click.native="snackbarPassNoCoincide = false">OK</v-btn>
    </v-snackbar>
...
<script>
  ...
  export default {
    ...
    data: () => ({
      ...
      reglas: {
        condiciones: [aceptaCondiciones => {
          if (!aceptaCondiciones) return "Si no aceptas los términos y condiciones no puedes registrarte";
          return true;
        }],
        pass: [pass => {
