<template>
  <v-dialog v-model="mostrar" persistent max-width="800">
    <v-card>
          ...
          <v-tab-item key="0">
            <v-card flat>
              <v-card-text>
                <v-layout row wrap>
                  <v-flex xs12>
                    <h3 class="headline"><strong>{{resumen.cantidadContado}}</strong>
                      {{resumen.cantidadContado <= 0 ? 'ventas' : resumen.cantidadContado === 1 ? 'venta' : 'ventas'}}
                      al contado, con valor
                      de <strong>{{resumen.contado | currency}}</strong>
                    </h3>
                    <h3 class="headline"><strong>{{resumen.cantidadApartados}}</strong>
                      {{resumen.cantidadApartados <= 0 || resumen.cantidadApartados > 1 ? 'apartados' : 'apartado'}},
                      con valor de <strong>{{resumen.apartadoYAbonado | currency}}</strong>.
                      Adeuda <strong>{{resumen.cuentasPendientes | currency}}</strong>
                    </h3>
                    <h3 class="headline">
                      Total comprado: <strong>{{resumen.contado + resumen.apartadoYAbonado | currency}}</strong>
                    </h3>
                  </v-flex>
                </v-layout>
              </v-card-text>
              ...
              <v-card-text>
                ...
                >
                  <template slot="items" slot-scope="props">
                    <td>{{ props.item.Numero }}</td>
                    <td>{{ props.item.Total | currency }}</td>
                    <td>{{ props.item.Abonado + props.item.Anticipo | currency }}</td>
                    <td>{{ props.item.Total - (props.item.Abonado + props.item.Anticipo) | currency }}</td>
                    <td>{{ props.item.Fecha | fechaExpresiva }}</td>
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn :loading="cargando" color="gray" flat="flat" @click.native="cerrarDialogo()">Cerrar</v-btn>
      </v-card-actions>
