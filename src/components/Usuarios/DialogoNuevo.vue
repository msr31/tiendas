    ...
    <v-card>
      ...
      <v-card-text>
        <v-alert type="info" :value="esParaNuevo">
          <strong>Se ha detectado que este es el primer uso del sistema. Por favor registre al usuario
            administrador</strong>
          <br>
          Recuerde elegir una contraseña segura y fácil de recordar, ya que de eso dependerá la seguridad de su cuenta.
        </v-alert>
        <v-form ref="formulario">
          <v-container fluid grid-list-md>
            <v-layout row wrap>
              <v-flex xs12>
                <v-text-field label="Nombre de usuario" type="text"
                              v-model="nuevoUsuario.nombre" :rules="reglas.nombre"
                              hint="Por ejemplo: maria, francisco, etcétera "
                              required></v-text-field>
              </v-flex>
              <v-flex xs12>
                <v-text-field label="Contraseña" type="password"
                              v-model="nuevoUsuario.pass" :rules="reglas.pass"
                              required></v-text-field>
              </v-flex>
              <v-flex xs12>
                <v-text-field label="Repetir contraseña" type="password"
                              v-model="nuevoUsuario.pass2" :rules="reglas.comprobarPass"
                              hint="Vuelva a escribir su contraseña"
                              required></v-text-field>
              </v-flex>
              <v-alert type="info" :value="!esParaNuevo">
                Recuerde elegir una contraseña segura, ya que de eso dependerá la seguridad de su cuenta.
                <br>
                No elija una contraseña como 123, 1245, su fecha de nacimiento o el nombre de su mascota
              </v-alert>
            </v-layout>
          </v-container>
        </v-form>
      ...
      <v-card-actions>
        ...
        <v-btn v-show="!esParaNuevo" color="gray" flat="flat" @click.native="cerrarDialogo()">Cerrar</v-btn>
      </v-card-actions>
    </v-card>
