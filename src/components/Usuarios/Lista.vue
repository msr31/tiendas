<template>
  <v-layout row wrap>
    ...
    <v-flex xs12>
      <v-data-table :loading="cargando" :headers="encabezados" :items="usuarios" hide-actions class="elevation-1">
        <template slot="items" slot-scope="props">
          <td>{{ props.item.Numero }}</td>
          <td>{{ props.item.Nombre }}</td>
          <td class="justify-center layout px-0">
            <v-btn title="Modificar permisos" icon class="mx-0" @click="cambiarPermisosDe(props.item)">
        ...
        <template slot="no-data">
          <v-alert :value="usuarios.length <= 0" color="info">
            <div class="text-xs-center">
              ...
              <p>No has registrado ningún usuario Agrega uno con el botón
                <v-icon>add</v-icon>
                de la esquina
              </p>
            </div>
