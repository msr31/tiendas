<template>
  <v-dialog v-model="mostrar" persistent max-width="800">
    <v-card>
      <v-card-title class="headline">Permisos de usuario {{usuario.Nombre}} (#{{usuario.Numero}})</v-card-title>
      <v-card-text>
        <v-list two-line>
          <template v-for="(permiso, indice) in permisos">
            <v-divider></v-divider>
            <v-list-tile style="height: auto;" avatar :key="indice">
              <v-list-tile-avatar style="height: auto;align-self: center; align-items: center">
                <v-checkbox
              ...
              <v-list-tile-content style="height: auto;">
                <v-list-tile-title style="white-space: normal; overflow: unset; height: auto;">
