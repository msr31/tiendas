<template>
  <v-flex xs12>
    ...
    <v-snackbar :timeout="5000" :bottom="true" v-model="snackbars.exportacion">
      Exportado con éxito
      <v-btn flat color="pink" @click.native="snackbars.exportacion = false">OK</v-btn>
    </v-snackbar>
    <v-snackbar :timeout="5000" :bottom="true" v-model="snackbars.noHayArchivos">
      No ha seleccionado ningún archivo
      <v-btn flat color="pink" @click.native="snackbars.noHayArchivos = false">OK</v-btn>
    </v-snackbar>
    <v-snackbar :timeout="5000" :bottom="true" v-model="snackbars.importacion">
      Importado con éxito
      <v-btn flat color="pink" @click.native="snackbars.importacion = false">OK</v-btn>
    </v-snackbar>
      ...
      <v-card>
        ...
        <v-card-text>
          <v-tabs centered color="cyan" dark icons-and-text>
            ...
            <v-tab href="#tab-1">Exportar
              <v-icon>cloud_download</v-icon>
            </v-tab>
            <v-tab href="#tab-2">Importar
              ...
              <v-card flat>
                <v-card-text>
                  ...
                  <v-alert outline type="info" v-model="mostrarDialogo">
                    <p>
                      El archivo será exportado en el directorio en donde se está ejecutando esta aplicación
                      <br>
                      Tenga en cuenta que el tiempo para generar el archivo dependerá de la velocidad de su equipo y de
                      la cantidad de productos
                      <br>
                      <strong>Advertencia: </strong> cualquier archivo exportado anteriormente será eliminado
                    </p>
                  </v-alert>
                  <v-btn :loading="cargando" small color="success" @click="exportar">Comenzar a exportar</v-btn>
                </v-card-text>
              </v-card>
            </v-tab-item>
              ...
              <v-card flat>
                <v-card-text>
                  ...
                  <v-container fluid grid-list-md>
                    <v-layout row wrap>
                      <v-flex xs12>
                        <input accept=".xlsx" ref="archivoImportar" name="archivoImportar" id="archivoImportar"
                               type="file">
                      </v-flex>
                    </v-layout>
                  </v-container>
                  ...
                  <v-alert outline type="info" v-model="mostrarDialogo">
                    <p>
                      Tenga en cuenta que el tiempo para importar los productos dependerá de la velocidad de su equipo y
                      de
                      la cantidad de los mismos
                      <br>
                      <strong>Advertencia: </strong> revise bien las opciones que seleccionó antes de importar, para
                      evitar resultados inesperados
                    </p>
                  </v-alert>
                  <v-btn :loading="cargando" small color="success" @click="importar">Comenzar a importar</v-btn>
                </v-card-text>
              </v-card>
            </v-tab-item>
          </v-tabs>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="gray" flat="flat" @click.native="mostrarDialogo = false">Cerrar</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-flex>
