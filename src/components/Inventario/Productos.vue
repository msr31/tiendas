<template>
  <v-layout>
    ...
    <formulario-editar-producto
      ...
      :mostrar="dialogos.editar" :producto="productoEditado"
    ></formulario-editar-producto>
    ...
    <dialogo-confirmar-eliminacion
      :mostrar="dialogos.confirmarEliminacion" :producto="productoParaEliminar"
      @cerrar-dialogo="dialogos.confirmarEliminacion = false"
      @producto-eliminado="onProductoEliminado()"
    ></dialogo-confirmar-eliminacion>

    <v-snackbar :timeout="2000" :bottom="true" v-model="snackbars.existenciaAumentada">
      Existencia aumentada
      <v-btn flat color="pink" @click.native="snackbars.existenciaAumentada = false">OK</v-btn>
    </v-snackbar>
    <v-snackbar :timeout="2000" :bottom="true" v-model="snackbars.existenciaRestada">
      Existencia restada
      <v-btn flat color="pink" @click.native="snackbars.existenciaRestada = false">OK</v-btn>
    </v-snackbar>
    <v-snackbar :timeout="2000" :bottom="true" v-model="snackbars.productoGuardado">
      Producto guardado correctamente
      <v-btn flat color="pink" @click.native="snackbars.productoGuardado = false">OK</v-btn>
    </v-snackbar>
    <v-snackbar :timeout="2000" :bottom="true" v-model="snackbars.cambiosProductoGuardados">
      Producto guardado correctamente
      <v-btn flat color="pink" @click.native="snackbars.productoGuardado = false">OK</v-btn>
    </v-snackbar>
    <v-tooltip top>
      <v-btn @click="mostrarDialogoParaAgregar()" fixed dark fab bottom fill-height slot="activator" open-delay="0"
             right color="indigo">
        <v-icon>add</v-icon>
      </v-btn>
