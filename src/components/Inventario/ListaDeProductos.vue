<template>
  <v-flex xs12>
    ...
    <v-flex xs12>
      <v-text-field label="Buscar un producto por su descripción" v-model="busqueda" prepend-icon="search" solo
                    clearable></v-text-field>
    </v-flex>
    <br>
    <div class="text-xs-center pt-2">
      <v-pagination v-model="paginacion.pagina" :length="numeroDePaginas"></v-pagination>
    </div>
    <v-data-table :loading="cargando" :headers="encabezados" :items="productos" hide-actions class="elevation-1">
      <template slot="items" slot-scope="props">
        ...
        <td>{{ props.item.PrecioVenta - props.item.PrecioCompra | currency}}</td>
        <td>{{ props.item.Existencia }}</td>
        <td>{{ props.item.Stock }}</td>
        <td class="justify-center layout px-0">
          <v-btn title="Crear una copia" icon class="mx-0" @click="duplicar(props.item)">
            <v-icon color="green darken-3">file_copy</v-icon>
          </v-btn>
          <v-btn title="Restar existencia" icon class="mx-0" @click="restarExistencia(props.item)">
            <v-icon color="orange darken-3">remove_circle</v-icon>
          </v-btn>
          <v-btn title="Aumentar existencia" icon class="mx-0" @click="aumentarExistencia(props.item)">
            <v-icon color="blue darken-3">add_circle</v-icon>
          </v-btn>
          <v-btn title="Editar/modificar" icon class="mx-0" @click="editar(props.item)">
            <v-icon color="amber darken-4">edit</v-icon>
          </v-btn>
          <v-btn title="Eliminar" icon class="mx-0" @click="eliminar(props.item)">
      ...
      <template slot="no-data">
        <v-alert v-show="!busqueda" :value="true" color="info">
          <div class="text-xs-center">
            ...
            <p>No has registrado ningún producto. Agrega uno con el botón
              <v-icon>add</v-icon>
              de la esquina
            </p>
          </div>
        ...
        <v-alert v-show="busqueda" :value="true" color="info" dark>
          <div class="text-xs-center">
            ...
            <p>No hay productos que coincidan con "
              <strong>{{busqueda}}</strong>"</p>
          </div>
        </v-alert>
      </template>
    </v-data-table>
    ...
    methods: {
      obtener() {
        ...
        if (this.busqueda) {
          ...
          HTTP_AUTH.get(
            `buscar/productos/${verdaderoOffset}/${
