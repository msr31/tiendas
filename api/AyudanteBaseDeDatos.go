...
func obtenerBaseDeDatosAPartirDeIdNegocio(idNegocio int) (*sql.DB, error) {
	...
	if ControladorBD == ControladorBDSQLite3 {
		cadenaConexion = CadenaConexionBDUsuarios + nombreBDAPartirDeNegocio(idNegocio) + ".db"
	} else {
		// Debe tener la forma usuario:contraseña@host/nombreBaseDeDatos
		nombreBaseDeDatos := nombreBDAPartirDeNegocio(idNegocio)
		cadenaConexion = fmt.Sprintf("%s:%s@%s/%s", UsuarioMySQL, PasswordMySQL, HostMySQL, nombreBaseDeDatos)
	}
