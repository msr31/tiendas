...
func configurarRutasDeClientes(enrutador *mux.Router) {
	enrutador.HandleFunc("/clientes/{offset}/{limite}", func(w http.ResponseWriter, r *http.Request) {
		...
		responderHttpExitoso(cc.todos(offset, limite), w, r)
	...
	enrutador.HandleFunc("/buscar/clientes/{nombre}/{offset}/{limite}", func(w http.ResponseWriter, r *http.Request) {
		variables := mux.Vars(r)
		offset, err := strconv.Atoi(variables["offset"])
		if err != nil {
			responderHttpConError(err, w, r)
		...
		responderHttpExitoso(cc.buscar(limite, offset, variables["nombre"]), w, r)
	}).Name("VerClientes").Methods(http.MethodGet)
