...
func formatearFechaSegunMotor(formato, fecha string) string {
	/*
		Para formatear fechas en distintos motores se hace distinto. En MySQL es:
		date_format(fecha, formato)
		y en SQLite3 es:
		strftime(formato, fecha)
		Entonces esta función se encarga de invocar a la función dependiendo del motor usado
		No es propenso a inyecciones SQL ya que los argumentos no son proporcionados por el usuario
	*/
	if ControladorBD == ControladorBDMySQL {
		return fmt.Sprintf(`date_format(%s, "%s")`, fecha, formato)
	} else {
		return fmt.Sprintf(`strftime("%s", %s)`, formato, fecha)
	}
}
...
func (d *DatosGraficasController) mesesEnLosQueHayRegistrosDeVentasAlContadoDependiendoDeAnio(anio int) []string {
	...
	defer db.Close()
	filas, err := db.Query(`SELECT DISTINCT `+formatearFechaSegunMotor("%m", "fecha")+` AS mes FROM ventas_contado WHERE fecha BETWEEN ? AND ?;`, anio, anio+1)
	if err != nil {
...
func (d *DatosGraficasController) aniosEnLosQueHayRegistrosDeVentasAlContado() []string {
	...
	defer db.Close()
	filas, err := db.Query(`SELECT DISTINCT ` + formatearFechaSegunMotor("%Y", "fecha") + ` AS anio FROM ventas_contado ORDER BY anio DESC;`)
	if err != nil {
...
func (d *DatosGraficasController) totalVentasPorAnio(anio string) []EtiquetaYValor {
	...
	*/
	...
	defer db.Close()
	filas, err := db.Query(`SELECT `+formatearFechaSegunMotor("%m", "fecha")+` mes, COALESCE(SUM(monto), 0) AS total FROM ventas_contado WHERE `+formatearFechaSegunMotor("%Y", "fecha")+`= ? GROUP BY mes;`, anio)
	if err != nil {
...
func (d *DatosGraficasController) totalVentasPorDiaEnMesYAnio(mes, anio string) []EtiquetaYValor {
	...
	*/
	...
	defer db.Close()
	filas, err := db.Query(`SELECT `+formatearFechaSegunMotor("%d", "fecha")+` AS dia, COALESCE(SUM(monto), 0) AS total FROM ventas_contado WHERE `+formatearFechaSegunMotor("%Y-%m", "fecha")+` = ? GROUP BY dia;`, mesYAnio)
	if err != nil {
