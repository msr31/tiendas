...
func (n *NegociosController) prepararDBMaestra() (bool, error) {
	...
	if ControladorBD == ControladorBDSQLite3 {
		nombreArchivo = NombreArchivoEsquemaSQLNegocios
	} else {
		nombreArchivo = NombreArchivoEsquemaSQLNegociosMySQL
	}
	// Leer lo que hay en el archivo sql
	bytesLeidos, err := ioutil.ReadFile(nombreArchivo)
	if err != nil {
		log.Printf("Error leyendo archivo %s: %v", nombreArchivo, err)
	}
...
func (n *NegociosController) prepararPrimerUso(negocio *Negocio) {
	...
	var nombreArchivo string
	if ControladorBD == ControladorBDSQLite3 {
		nombreArchivo = NombreArchivoEsquemaSQLSistemas
	} else {
		nombreArchivo = NombreArchivoEsquemaSQLSistemasMySQL
	}
	bytesLeidos, err := ioutil.ReadFile(nombreArchivo)
	if err != nil {
		log.Printf("Error leyendo archivo %s: %v", nombreArchivo, err)
	...
	// Luego los permisos
	var nombreArchivoEsquemaInit string
	if ControladorBD == ControladorBDSQLite3 {
		nombreArchivoEsquemaInit = NombreArchivoEsquemaSQLInit
	} else {
		nombreArchivoEsquemaInit = NombreArchivoEsquemaSQLInitMySQL
	}
	bytesLeidos, err = ioutil.ReadFile(nombreArchivoEsquemaInit)
	if err != nil {
		log.Printf("Error leyendo archivo %s: %v", nombreArchivoEsquemaInit, err)
	}
